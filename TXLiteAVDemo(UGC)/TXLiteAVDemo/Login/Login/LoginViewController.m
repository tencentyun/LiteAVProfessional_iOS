...
@property (nonatomic, strong) UIImageView *inputBackImage; //输入框背景
...
- (void)setupUI {
	...
	NSString *sdvVersionStr;
#if LIVE
	sdvVersionStr = [V2TXLivePremier getSDKVersionStr];
#else
	sdvVersionStr = [TXLiveBase getSDKVersionStr];
	if (!sdvVersionStr) {
		sdvVersionStr = @"1.0.0";
	}
#endif
	self.versionTip.text = [NSString stringWithFormat:@"TRTC v %@ %@",version, sdvVersionStr];
