...
@interface LiveRoomPusherViewController () <MLVBLiveRoomDelegate, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource> {
    UIView *             _pusherView;
...
- (void)initUI {
    ...
    [_msgListView setUserInteractionEnabled:NO];
    ...
    _vBeauty = [TCBeautyPanel beautyPanelWithFrame:CGRectMake(0, self.view.frame.size.height - controlHeight, self.view.frame.size.width, controlHeight) SDKObject:_liveRoom];
...
- (void)relayout {
    ...
    if (_pkStatus == PKStatus_BEING) {
        ...
        _msgListView.frame = CGRectMake(10, videoViewHeight + 6, self.view.width - 20, self.view.height - videoViewHeight - self.view.width / 10 - 60);
               ...
               completion:^(int errCode, NSString *errMsg) {
                   ...
                   dispatch_async(dispatch_get_main_queue(), ^{
                       if (errCode == 0) {
                           ...
                           UIAlertAction *    action     = [UIAlertAction actionWithTitle:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomPusher.toopen")
                                                                            style:UIAlertActionStyleDefault
                                                                          handler:^(UIAlertAction *_Nonnull action) {
                                                                              [self.navigationController popViewControllerAnimated:YES];
                                                                              [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"https://buy.cloud.tencent.com/avc"]];
                                                                          }];
                           UIAlertAction *    confirm    = [UIAlertAction actionWithTitle:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomPusher.confirm")
                                                                             style:UIAlertActionStyleDefault
                                                                           handler:^(UIAlertAction *_Nonnull action) {
                                                                               [self.navigationController popViewControllerAnimated:YES];
                                                                           }];
                           ...
                           [self alertTips:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomPusher.failedtocreatealivebooth")
                                       msg:errMsg
                                completion:^{
                                    [self.navigationController popViewControllerAnimated:YES];
                                }];
...
- (void)onRoomDestroy:(NSString *)roomID {
    [self alertTips:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomNew.prompt")
                msg:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomPlayer.liveroomhasbeendisbanded")
         completion:^{
             [self.navigationController popViewControllerAnimated:YES];
         }];
}
...
- (void)onAnchorEnter:(MLVBAnchorInfo *)anchorInfo {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self->_liveRoom startRemoteView:anchorInfo
            view:playerView
            onPlayBegin:^{
...
- (void)onRequestJoinAnchor:(MLVBAnchorInfo *)anchor reason:(NSString *)reason {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomNew.prompt")
                                                                                 message:msg
                                                                          preferredStyle:UIAlertControllerStyleAlert];
        [alertController addAction:[UIAlertAction actionWithTitle:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomPusher.refusedto")
                                                            style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction *_Nonnull action) {
                                                              self->_hasPendingRequest = NO;
                                                              [self->_liveRoom responseJoinAnchor:userID
                                                                                            agree:NO
                                                                                           reason:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomPusher.anchordoesnotyourconnection")];
                                                          }]];
        [alertController addAction:[UIAlertAction actionWithTitle:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomPusher.accept")
                                                            style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction *_Nonnull action) {
                                                              self->_hasPendingRequest = NO;
...
- (void)sendPKRequest:(MLVBAnchorInfo *)anchor {
    ...
    if (_pkStatus == PKStatus_IDLE) {     // 空闲状态
        ...
        __weak __typeof(self) wself = self;
        [_liveRoom requestRoomPK:anchor.userID
                      completion:^(int errCode, NSString *errMsg, NSString *streamUrl) {
