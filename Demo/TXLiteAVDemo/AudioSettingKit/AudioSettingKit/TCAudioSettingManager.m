...
- (void)setBGMRate:(CGFloat)rate {
    if (_bgmID != 0) {
        [self.manager setMusicSpeedRate:_bgmID speedRate:rate];
    }
}
...
- (void)setProgress:(CGFloat)progress {
    if (_bgmID != 0) {
        [self.manager seekMusicToPosInMS:_bgmID pts:progress * self.duration];
    }
}
...
- (void)playMusicWithPath:(NSString *)path bgmID:(NSInteger)bgmID {
    ...
    params.path               = path;
    params.loopCount          = 0;
    self.duration = [self.manager getMusicDurationInMS:path];
    __weak typeof(self) weakSelf = self;
    [self.manager startPlayMusic:params
