...
- (void)setupUI {
    ...
    self.sendButton = [UIButton trtc_cellButtonWithTitle:TRTCLocalize(@"Demo.TRTC.Live.send")];
...
- (instancetype)initWithTitle:(NSString *)title placeHolder:(NSString *)placeHolder action:(void (^)(NSString *_Nullable content))action {
    return [self initWithTitle:title placeHolder:placeHolder content:nil actionTitle:TRTCLocalize(@"Demo.TRTC.LiveRoom.send") action:action];
}
