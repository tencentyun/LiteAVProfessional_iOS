...
- (void)makeCustomRegistrition {
    [self.tableView registerClass:TRTCRemoteUserItem.bindedCellClass forCellReuseIdentifier:TRTCRemoteUserItem.bindedCellId];
}
...
- (NSMutableArray *)buildMemberItems {
    ...
    for (TRTCVideoView *view in [self.trtcCloudManager.viewDic allValues]) {
        if ([view.userId isEqualToString:self.trtcCloudManager.userId]) {
            continue;
        }
