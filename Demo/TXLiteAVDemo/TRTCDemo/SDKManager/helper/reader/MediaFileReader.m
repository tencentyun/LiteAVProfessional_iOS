...
@implementation MediaFileReader
    ...
    if (audioFMT != nil && audioFMT.count > 0) {
        ...
        if (fmtDesc != nil) {
            ...
            if (audioDesc != nil) {
                ...
                if (audioDesc->mBytesPerFrame != 0)
                    _audioBytesPerFrame = audioDesc->mBytesPerFrame;
                else
                    _audioBytesPerFrame = 2 * _audioChannels;
            }
        }
    }
}
...
- (AVAssetReaderOutput *)getTrackOutput:(NSInteger)index isVideo:(BOOL)isVideo {
    ...
    if (isVideo) {
        ...
        NSDictionary *settingA = @{(id)AVFormatIDKey : @(kAudioFormatLinearPCM), (id)AVLinearPCMBitDepthKey : @(16), (id)AVLinearPCMIsFloatKey : @(false)};
...
- (void)readVideoFrameFromTime:(float)startTimef toTime:(float)endTime fps:(float)fps readOneFrame:(ReadOneFrame)readOneFrame readFinished:(ReadFinished)readFinished {
...
- (void)readAudioFrameFromTime:(float)startTimef toTime:(float)endTime readOneFrame:(ReadOneFrame)readOneFrame readFinished:(ReadFinished)readFinished {
    __block float         startTime = startTimef;
    __weak __typeof(self) weakSelf  = self;
    dispatch_async(_audioReadedQueue, ^{
        ...
        if (trackOutput == nil || reader == nil || !strongSelf.audioCanRead) return;
        [reader startReading];
        CMSampleBufferRef sample = [trackOutput copyNextSampleBuffer];
        CMTime            time   = kCMTimeZero;
        while (sample != nil) {
            if (!strongSelf.audioCanRead) {
                CFRelease(sample);
                [reader cancelReading];
                return;
            }
            ...
            if (strongSelf.resetAudioReader) {
                [self cutAudioFromTime:(int)pts toTime:endTime];
                startTime   = pts;
                trackOutput = [strongSelf getTrackOutput:0 isVideo:NO];
                [reader cancelReading];
                reader = [strongSelf getAssetReader:0 isVideo:NO trackOutput:trackOutput];
                [reader startReading];
                strongSelf.resetAudioReader = NO;
            }
            sample = [trackOutput copyNextSampleBuffer];
        }
        [reader cancelReading];
        if (strongSelf.audioCanRead) readFinished();
    });
}
