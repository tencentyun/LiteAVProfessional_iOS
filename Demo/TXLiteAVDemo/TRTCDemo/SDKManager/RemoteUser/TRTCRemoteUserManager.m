...
- (void)addUser:(NSString *)userId roomId:(NSString *)roomId {
    ...
    TRTCRemoteUserConfig *config = [[TRTCRemoteUserConfig alloc] init];
...
- (void)enableAutoReceiveAudio:(BOOL)autoReceiveAudio autoReceiveVideo:(BOOL)autoReceiveVideo {
    self.autoReceivesAudio = autoReceiveAudio;
    [self.trtc setDefaultStreamRecvMode:autoReceiveAudio video:autoReceiveVideo];
}
...
- (void)setUser:(NSString *)userId fillMode:(TRTCVideoFillMode)fillMode {
    self.users[userId].renderParams.fillMode = fillMode;
    [self updateRenderParams:userId];
}

- (void)setUser:(NSString *)userId rotation:(TRTCVideoRotation)rotation {
    self.users[userId].renderParams.rotation = rotation;
    [self updateRenderParams:userId];
}

- (void)setUser:(NSString *)userId isMirrorEnabled:(BOOL)isEnabled {
    self.users[userId].renderParams.mirrorType = isEnabled ? TRTCVideoMirrorTypeEnable : TRTCVideoMirrorTypeDisable;
    [self updateRenderParams:userId];
}

- (void)updateRenderParams:(NSString *)userId {
    [self.trtc setRemoteRenderParams:userId streamType:TRTCVideoStreamTypeBig params:self.users[userId].renderParams];
}
