...
@interface ScreenPushViewController () <AddressBarControllerDelegate,
                                        ScanQRDelegate,
#ifndef DISABLE_VOD
                                        TXVodPlayListener,
#endif
...
@property(nonatomic, retain) UIButton*             micControlButton;
...
- (void)dealloc {
    ...
    [[self.livePusher getAudioEffectManager] setVoiceVolume:100];
...
- (void)startMonitoringNetwork {
    ...
    [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        if (status == AFNetworkReachabilityStatusNotReachable) {
            UIAlertController* alertVC = [UIAlertController alertControllerWithTitle:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomNew.prompt")
                                                                             message:LivePlayerLocalize(@"LiveLinkMicDemoOld.Live.LinkMicNew.checknetworkandtry")
                                                                      preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction*     action  = [UIAlertAction actionWithTitle:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomList.determine")
                                                           ...
                                                           handler:^(UIAlertAction* _Nonnull action) {
                                                               if (![strongSelf.btnReplaykit.currentTitle isEqualToString:LivePlayerLocalize(@"LivePusherDemo.ScreenPush.startpushstream")]) {
                                                                   [strongSelf.livePusher stopPush];
...
- (void)initUI {
    ...
    NSArray* resolutions =
        @[ LivePlayerLocalize(@"LivePusherDemo.PushSetting.superclear"), LivePlayerLocalize(@"LivePusherDemo.PushSetting.hd"), LivePlayerLocalize(@"LivePusherDemo.PushSetting.standarddefinition") ];
    ...
    self.micControlButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.micControlButton.center = CGPointMake(self.view.center.x + 100, self.resolutionSelector.bottom + 60);
    self.micControlButton.bounds = CGRectMake(0, 0, 120, 50);
    self.micControlButton.titleLabel.adjustsFontSizeToFitWidth = true;
    self.micControlButton.backgroundColor = UIColor.lightTextColor;
    self.micControlButton.layer.cornerRadius = 5;
    [self.micControlButton setTitleEdgeInsets:UIEdgeInsetsMake(0, 5, 0, 5)];
    [self.micControlButton setTitle:LivePlayerLocalize(@"LivePusherDemo.ScreenPush.openmic") forState:UIControlStateNormal];
    [self.micControlButton setTitle:LivePlayerLocalize(@"LivePusherDemo.ScreenPush.closemic" ) forState:UIControlStateSelected];
    [self.micControlButton addTarget:self action:@selector(clickMicControlButton:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:self.micControlButton];
...
- (void)addressBarControllerTapCreateURL:(AddressBarController*)controller {
    ...
    [TCHttpUtil asyncSendHttpRequest:@"get_test_pushurl"
                          ...
                          HTTPMethod:@"GET"
                             ...
                             handler:^(int result, NSDictionary* resultDict) {
                                 if (result != 0) {
                                     ...
                                     controller.qrStrings =
                                         @[ c(@"rtmp", rtmpPlayUrl), c(@"flv", flvPlayUrl), c(@"hls", hlsPlayUrl), c(LivePlayerLocalize(@"LivePusherDemo.CameraPush.lowlatency"), accPlayUrl) ];

                                     NSString*     playUrls   = LocalizeReplaceFourCharacter(LivePlayerLocalize(@"LivePusherDemo.CameraPush.rtmpaddressxxflvaddressyyhlsaddresszz"),
                                                                                       [NSString stringWithFormat:@"%@", rtmpPlayUrl], [NSString stringWithFormat:@"%@", flvPlayUrl],
                                                                                       [NSString stringWithFormat:@"%@", hlsPlayUrl], [NSString stringWithFormat:@"%@", accPlayUrl]);
...
- (void)clickStartReplaykit:(UIButton*)btn {
    if ([UIDevice currentDevice].systemVersion.floatValue < 11.0) {
        UIAlertController* alert   = [UIAlertController alertControllerWithTitle:LivePlayerLocalize(@"LivePusherDemo.ScreenPush.tencentcloudpushstream")
                                                                       message:LivePlayerLocalize(@"LivePusherDemo.ScreenPush.systempleaseupgrade")
                                                                preferredStyle:UIAlertControllerStyleAlert];
    ...
    if (isStart) {
        ...
        if (!isCaptured) {
            ...
            UIAlertController* alert   = [UIAlertController alertControllerWithTitle:LivePlayerLocalize(@"LivePusherDemo.ScreenPush.tencentcloudpushstream")
                                                                           message:message
                                                                    preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction*     action1 = [UIAlertAction actionWithTitle:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomList.determine")
                                                              style:UIAlertActionStyleDefault
                                                            handler:^(UIAlertAction* _Nonnull action){
                                                            }];
            ...
            UIAlertController* alert   = [UIAlertController alertControllerWithTitle:LivePlayerLocalize(@"LivePusherDemo.ScreenPush.tencentcloudpushstream")
                                                                           message:LivePlayerLocalize(@"LivePusherDemo.ScreenPush.turnonscreenrecordingpushstreams")
                                                                    preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction*     action1 = [UIAlertAction actionWithTitle:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomList.determine")
                                                              style:UIAlertActionStyleDefault
                                                            handler:^(UIAlertAction* _Nonnull action) {
                                                                ...
                                                                [self.livePusher setAudioQuality:V2TXLiveAudioQualityDefault];
                                                                if (@available(iOS 11.0, *)) {
                                                                    [self.livePusher startScreenCapture:kReplayKit2AppGroupId];
                                                                }
                                                                [self refreshResolutionAndRotation];
                                                                [self.livePusher startMicrophone];
                                                                [[self.livePusher getAudioEffectManager] setVoiceVolume:self.micControlButton.selected ? 100 : 0];
        ...
        UIAlertController* alert   = [UIAlertController alertControllerWithTitle:LivePlayerLocalize(@"LivePusherDemo.ScreenPush.tencentcloudpushstream")
                                                                       message:LivePlayerLocalize(@"LivePusherDemo.ScreenPush.closescreenpushstream")
                                                                preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction*     action1 = [UIAlertAction actionWithTitle:LivePlayerLocalize(@"LiveLinkMicDemoOld.RoomList.determine")
                                                          style:UIAlertActionStyleDefault
                                                        handler:^(UIAlertAction* _Nonnull action) {
                                                            [self.livePusher stopPush];
                                                            if (self.micControlButton.selected) {
                                                                [self clickMicControlButton:self.micControlButton];
                                                            }
...
- (void)refreshResolutionAndRotation {
    ...
    videoEncoderParam.videoResolutionMode = resMode;
    [self.livePusher setVideoQuality:videoEncoderParam];
}

- (void)clickMicControlButton:(UIButton *)btn {
    btn.selected = !btn.selected;
    // 录屏直播静音人声采集，需要采用设置音量的方式实现
    // APP音量是混合到麦克风采集中的，直接静音麦克风，会导致系统采集的APP音量也被静音
    [[self.livePusher getAudioEffectManager] setVoiceVolume:btn.selected ? 100 : 0];
}
...
- (void)sendLocalNotificationToHostAppWithTitle:(NSString*)title msg:(NSString*)msg userInfo:(NSDictionary*)userInfo {
    ...
    UNNotificationRequest* request = [UNNotificationRequest requestWithIdentifier:@"ReplayKit2Demo" content:content trigger:trigger];
