...
#define RTCLogSimple()        NSLog(@"[%@ %p %s %d]", NSStringFromClass(self.class), self, __func__, __LINE__);
#define RTCLog(_format_, ...) NSLog(@"[%@ %p %s %d] %@", NSStringFromClass(self.class), self, __func__, __LINE__, [NSString stringWithFormat:_format_, ##__VA_ARGS__]);

@interface                                                   RTCPlayerViewController () <V2TXLivePlayerObserver, RTCPlayerSettingViewControllerDelegate>
...
@property(nonatomic, strong) RTCPlayerSettingViewController *settingContainer;
...
- (void)addSettingContainerView {
    self.settingContainer          = [[RTCPlayerSettingViewController alloc] initWithHostVC:self muteAudio:NO muteVideo:NO logView:NO player:self.player];
...
- (void)setUrl:(NSString *)url {
    ...
    NSDictionary *params = [RTCLiveUtils parseURLParametersAndLowercaseKey:url];
    _userId              = params[@"userid"];
    if ([RTCLiveUtils isTRTCUrl:url]) {
        self.title = [NSString stringWithFormat:@"%@（%@）", V2Localize(@"MLVB.lebLauncher.title"), params[@"strroomid"]];
    } else {
        self.title = V2Localize(@"MLVB.lebLauncher.title");  //[NSString stringWithFormat:@"RTC拉流（%@_%@）", params[@"strroomid"], params[@"remoteuserid"]];
    }
}
...
- (V2TXLiveCode)stopPlay {
    RTCLogSimple() if ([NSThread isMainThread]) { return [self startPlayInner:NO]; }
    else {
        __block V2TXLiveCode result = V2TXLIVE_OK;
        dispatch_sync(dispatch_get_main_queue(), ^{
...
- (V2TXLiveCode)startPlayInner:(BOOL)start {
    ...
    if (start == self.player.isPlaying) {
        ...
        RTCLog(@"startPlay ignored, already %@", message);
        return V2TXLIVE_OK;
    }
    if (start) {
        RTCLog(@"startPlay.") self.hasRecvFirstFrame = NO;
        ...
        if (result == V2TXLIVE_OK) {
            ...
            __weak RTCPlayerViewController *weakSelf = self;
        ...
        RTCLog(@"stopPlay.") result = [self.player stopPlay];
        if (result == V2TXLIVE_OK) {
            if (self.delayBlock) {
                dispatch_block_cancel(self.delayBlock);
                self.delayBlock = nil;
...
#pragma mark - RTCPlayerSettingViewControllerDelegate
...
- (void)RTCPlayerSettingVC:(id)container enableVolumeEvaluation:(BOOL)isEnable {
...
+ (NSString *)convertPushUrl:(NSString *)pushUrl {
    ...
    if (!defaultUserId) {
        RTCLog(@"faild. no default userid.");
        return pushUrl;
    }
    NSArray *urlComponents = [pushUrl componentsSeparatedByString:@"?"];
    if ([urlComponents count] != 2) {
        RTCLog(@"faild. invalid url:%@", pushUrl);
        return pushUrl;
    }
    ...
    if ([pushUrl hasPrefix:@"trtc://"] && [pushUrl containsString:@"/push/"]) {
        ...
        RTCLog(@"succ. playUrl:%@ originUrl:%@", playUrl, pushUrl);
        return playUrl;
...
- (void)onError:(id<V2TXLivePlayer>)player code:(V2TXLiveCode)code message:(NSString *)msg extraInfo:(NSDictionary *)extraInfo {
    ...
    RTCLog(@"code:%ld msg:%@ extraInfo:%@", (long)code, msg, extraInfo);
}
