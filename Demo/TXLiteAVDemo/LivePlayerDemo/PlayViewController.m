...
@interface PlayViewController () <V2TXLivePlayerObserver, AddressBarControllerDelegate, ScanQRDelegate, UITextFieldDelegate> {
...
@property(nonatomic, weak) PlayCacheStrategyView *cacheStrategyView;
@property(nonatomic, assign) ENUM_TYPE_CACHE_STRATEGY currentStrategy;
@end
...
- (void)viewDidLoad {
    ...
    self.currentStrategy = CACHE_STRATEGY_AUTO;
...
- (void)initUI {
    ...
    _btnPlay        = [self createButton:@"start" action:@selector(clickPlay:) center:CGPointMake(startSpace + ICON_WITH / 2 + centerInterVal * index++, iconY) size:iconSize];
    ...
    _btnStrategy    = [self createButton:@"cache_time" action:@selector(clickCacheStrategy:) center:CGPointMake(startSpace + ICON_WITH / 2 + centerInterVal * index++, iconY) size:iconSize];
...
- (void)clickCacheStrategy:(UIButton *)btn {
    if (_cacheStrategyView == nil) {
        CGFloat height = 200;
        PlayCacheStrategyView *strategyView = [[PlayCacheStrategyView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(btn.frame) - height, self.view.frame.size.width, height)];
        [self.view addSubview:strategyView];
        self.cacheStrategyView = strategyView;
        
        strategyView.titleText = LivePlayerLocalize(@"LivePlayerDemo.PlayViewController.bufferingstrategy");
        strategyView.closeText = LivePlayerLocalize(@"LivePusherDemo.CameraPush.off");
        strategyView.dataSource = @[
            LivePlayerLocalize(@"LivePlayerDemo.PlayViewController.speed"),
            LivePlayerLocalize(@"LivePlayerDemo.PlayViewController.smooth"),
            LivePlayerLocalize(@"LivePlayerDemo.PlayViewController.auto")
        ];
        __weak typeof(self)weakSelf = self;
        self.cacheStrategyView.didSelectIndex = ^(NSInteger index) {
            weakSelf.currentStrategy = index + 1;
        };
    }
    [self.cacheStrategyView show];
}

- (void)setCacheStrategy:(ENUM_TYPE_CACHE_STRATEGY)type {
    switch (type) {
        case CACHE_STRATEGY_FAST:
            [_player setCacheParams:1.0 maxTime:1.0];
            break;
        case CACHE_STRATEGY_SMOOTH:
            [_player setCacheParams:5.0 maxTime:5.0];
            break;
        case CACHE_STRATEGY_AUTO:
            [_player setCacheParams:1.0 maxTime:5.0];
            break;
        default:
            break;
    }
...
- (BOOL)startPlay {
    ...
    [self setCacheStrategy:self.currentStrategy]; 
    // 开始推流
    NSString *charaters = @"!@#$^&%*+,;='\"`<>()[]{}\\| ";
    NSCharacterSet *allowedCharacterSet = [[NSCharacterSet characterSetWithCharactersInString:charaters] invertedSet];
    V2TXLiveCode ret = [_player startPlay:[playUrl stringByAddingPercentEncodingWithAllowedCharacters:allowedCharacterSet]];
    if (ret != V2TXLIVE_OK) {
...
- (void)fetchAccURL {
    ...
    [TCHttpUtil asyncSendHttpRequest:@"get_test_rtmpaccurl"
