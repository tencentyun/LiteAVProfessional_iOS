...
#define TX_DEPRECAETD_AUDIO_EFFECT_API __deprecated_msg("Use getAudioEffectManager instead.")

/// @defgroup TXLivePush_ios TXLivePush
/// 直播推流类
/// @{
...
/**
 * 5.2 获取音效管理类 TXAudioEffectManager, 用于管理BGM, 短音效和人声特效
 */
 ...
 * 5.3 设置麦克风采集音量大小回调接口
 *
 * @param volumeEvaluationListener 麦克风采集音量大小回调接口，音量取值范围0 - 100
 */
...
/**
 * 6.1 录制回调接口，详见 "TXLiveRecordTypeDef.h" 中的 TXLiveRecordListener 定义。
 */
...
/**
 * 6.3 结束录制短视频，当停止推流后，如果视频还在录制中，SDK 内部会自动结束录制。
 ...
 *       2. SDK 内部有简单的帧率控制，如果发送太快时 SDK 会自动丢弃多余的帧；如果超时不发送，SDK 会不断地重复发送的最后一帧。
 *
...
/**
 * 7.2 自定义音频采集，向 SDK 发送自己采集的音频 PCM 数据。
 *
 ...
 */
#if TARGET_OS_IPHONE
- (void)sendAudioSampleBuffer:(CMSampleBufferRef)sampleBuffer withType:(RPSampleBufferType)sampleBufferType API_AVAILABLE(ios(10.0));
#endif
...
/**
 * 8.1 发送 SEI 消息，播放端（TXLivePlayer）通过 onPlayEvent（EVT_PLAY_GET_MESSAGE）来接收该消息。
 *
...
/**
 * 8.2 打开包含视频状态信息的调试浮层，该浮层一般用于 SDK 调试期间，外发版本请不要打开。
 */
- (void)showVideoDebugLog:(BOOL)isShow;

/**
 * 8.3 设置调试浮层在视频 view 上的位置。
 */
...
#if TARGET_OS_IPHONE
/**
 * 9.1 开始全系统的屏幕分享（该接口支持 iOS 11.0 及以上的 iPhone 和 iPad）
 *
 * 该接口支持共享整个 iOS 系统的屏幕，可以实现类似腾讯会议的全系统级的屏幕分享。
 *
 * @param appGroup 主 App 与 Broadcast 共享的 Application Group Identifier，可以指定为 nil。
 */
 ...
 * 10.2 设置指定素材滤镜特效
 *
 * @deprecated v7.2 版本弃用，请使用 TXBeautyManager 设置滤镜
 */
- (void)setFilter:(TXImage *)image TX_DEPRECAETD_BEAUTY_API;
 ...
 * @deprecated v7.2 版本弃用，请使用 TXBeautyManager 设置滤镜
 */
- (void)setSpecialRatio:(float)specialValue TX_DEPRECAETD_BEAUTY_API;

/**
 * 10.4 设置大眼级别（企业版有效，其它版本设置此参数无效）
 *
 * @deprecated v6.9 版本弃用，请使用 TXBeautyManager 设置美颜功能
 */
...
/**
 * 10.6 设置 V 脸级别（企业版有效，其它版本设置此参数无效）
 *
 * @deprecated v6.9 版本弃用，请使用 TXBeautyManager 设置美颜功能
 */
...
/**
 * 10.7 设置下巴拉伸或收缩（企业版有效，其它版本设置此参数无效）
 *
 * @deprecated v6.9 版本弃用，请使用 TXBeautyManager 设置美颜功能
 */
...
/**
 * 10.10 设置绿幕背景视频（企业版有效，其它版本设置此参数无效）
 *
 * @deprecated v7.2 版本弃用，请使用 TXBeautyManager 设置绿幕背景视频
 */
- (void)setGreenScreenFile:(NSURL *)file TX_DEPRECAETD_BEAUTY_API;

/**
 * 10.11 选择使用哪一款 AI 动效挂件（企业版有效，其它版本设置此参数无效）
 *
 * @deprecated v6.9 版本弃用，请使用 TXBeautyManager 设置美颜功能
 */
...
/**
 * 10.12 设置动效静音（企业版有效，其它版本设置此参数无效）
 *
 * @deprecated v6.9 版本弃用，请使用 TXBeautyManager 设置美颜功能
 */
 ...
 * @return YES：成功；NO：失败。
 */
- (BOOL)playBGM:(NSString *)path TX_DEPRECAETD_AUDIO_EFFECT_API;
...
withProgressNotify:(void (^)(NSInteger progressMS,NSInteger durationMS))progressNotify
 andCompleteNotify:(void (^)(NSInteger errCode))completeNotify TX_DEPRECAETD_AUDIO_EFFECT_API;
 ...
 */
- (BOOL)stopBGM TX_DEPRECAETD_AUDIO_EFFECT_API;
 ...
 */
- (BOOL)pauseBGM TX_DEPRECAETD_AUDIO_EFFECT_API;
 ...
 */
- (BOOL)resumeBGM TX_DEPRECAETD_AUDIO_EFFECT_API;

/**
 * 10.18 获取背景音乐文件的总时长，单位是毫秒
 *
 * @deprecated v7.3 版本弃用，请使用 TXAudioEffectManager getMusicDurationInMS 接口
 * @param path 音乐文件路径，如果 path 为空，那么返回当前正在播放的背景音乐的时长。
 */
- (int)getMusicDuration:(NSString *)path TX_DEPRECAETD_AUDIO_EFFECT_API;

/**
 * 10.19 设置混音时背景音乐的音量大小，仅在播放背景音乐混音时使用。
 *
 * @deprecated v7.3 版本弃用，请使用 TXAudioEffectManager setMusicVolume 接口
 * @param volume 音量大小，1为正常音量，范围是0 - 1之间的浮点数。
 * @return YES：成功；NO：失败。
 */
- (BOOL)setBGMVolume:(float)volume TX_DEPRECAETD_AUDIO_EFFECT_API;

/**
 * 10.20 设置混音时麦克风音量大小，仅在播放背景音乐混音时使用。
 *
 * @deprecated v7.3 版本弃用，请使用 TXAudioEffectManager setVoiceVolume 接口
 * @param volume 音量大小，1为正常音量，范围是0 - 1之间的浮点数。
 * @return YES：成功；NO：失败。
 */
- (BOOL)setMicVolume:(float)volume TX_DEPRECAETD_AUDIO_EFFECT_API;

/**
 * 10.21 调整背景音乐的音调高低
 *
 * @deprecated v7.3 版本弃用，请使用 TXAudioEffectManager setMusicPitch 接口
 * @param pitch 音调，默认值是0.0f，范围是-1 - 1之间的浮点数；
 * @return YES：成功；NO：失败。
 */
- (BOOL)setBGMPitch:(float)pitch TX_DEPRECAETD_AUDIO_EFFECT_API;
 ...
 * @return YES：成功；NO：失败。
 */
- (BOOL)setReverbType:(TXReverbType)reverbType TX_DEPRECAETD_AUDIO_EFFECT_API;
 ...
 * @return YES：成功；NO：失败。
 */
- (BOOL)setVoiceChangerType:(TXVoiceChangerType)voiceChangerType TX_DEPRECAETD_AUDIO_EFFECT_API;
 ...
 */
- (BOOL)setBGMPosition:(NSInteger)position TX_DEPRECAETD_AUDIO_EFFECT_API;
